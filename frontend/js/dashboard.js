...
function displayProducts(products) {
    ...
    productsList.innerHTML = products.map(product => {
        ...
        if (product.images && product.images.length > 0) {
            ...
            // Si l'URL est déjà complète (Cloudinary), on l'utilise directement
        ...
        return `
            <div class="product-item">
                <div class="product-item-info">
                    ...
                    ${imageUrl ? `<img src="${imageUrl}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px; margin-top: 5px;" onerror="this.onerror=null; this.src='';">` : '<span style="font-size: 0.7rem; color: #8B7A68;">Pas d\'image</span>'}
...
async function updateOrderStatus(orderId, newStatus) {
    ...
    try {
        ...
        if (response.ok) {
            let message = '';
            switch(newStatus) {
                case 'confirmed': message = 'Commande confirmée'; break;
